...
function convertPixelToSvgCoord(event) {
    let p = event.currentTarget.ownerSVGElement.createSVGPoint();
    p.x = event.clientX;
    p.y = event.clientY;
    return p.matrixTransform(event.currentTarget.getScreenCTM().inverse());
}
#+end_src

This function doesn't handle other SVG transforms, but see [[#parent-transform][the parent transform section]] for a version that does.
