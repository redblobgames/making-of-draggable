...
Note that ~.preventDefault()~ on =pointerstart= isn't what we need; it has to be =touchstart=. In Canvas and WebGL, check that the pointer is on the circle and conditionally prevent default.
