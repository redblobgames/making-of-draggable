...
digraph {
    ...
    rankdir=LR;
    "not\ndragging" -> dragging [label = "pointerdown"];
    dragging -> dragging [label = "pointermove"];
    dragging -> "not\ndragging" [label = "pointerup"];
}
#+end_src

#+results[ed91ff1542b1b6a147051f3530e955b0e1020940]:
...
#+begin_export html
<pre id="final-code" data-code="pointer" data-show="capture noscroll left offset"/>
<div id="final-code-options">
<label><input data-show="noselect nosystemdrag" type="checkbox" /> Text inside draggable</label>
<label><input data-show="pointerid" type="checkbox" /> Multiple fingers on same draggable</label>
<label><input data-show="nopropagate" type="checkbox" /> Nested draggable elements</label>
<label><input data-show="class" type="checkbox" /> Set class for styling</label>
</div>
#+end_export
...
This recipe is for the /input/ 
@@html:<em style="background:#e6d4c3"> event handler </em>@@,
and it needs to be connected to a 
@@html:<em style="background:#cfd6e6"> state handler </em>@@ to handle /output/.
*See the [[href:examples.html][examples page]]* for complete code you can use, including when I'm /not/ dragging an object around, like dragging this number left/right:
@@html:<input id="scrubbable-number" pattern="[0-9]+" />@@,
or [[href:targets.html#canvas-drag-to-paint][painting on a canvas]], or [[href:/articles/curved-paths/making-of.html][constrained dragging]].

[[./build/diagram-state-and-event-handlers.svg]]
...
1. [[href:./targets.html][List of edge cases and list of dragging tests]]
1. [[href:/x/2251-draggable/][Test page to log the events, and unorganized notes]]
1. [[href:/x/1845-draggable/][How I handled dragging events 2015–2018]]

Note that this is /not/ the HTML [[href:/x/2310-drag-drop-api/][Drag and Drop API]], which involves dragging an element /onto/ another element. For my diagrams, I'm dragging but /not/ dropping, and the scrubbable number example shows how I'm not necessarily even moving something around. So I need to read the mouse/touch events directly.
...
digraph {
    ...
    rankdir=LR;
    "not\ndragging" -> "not\ndragging" [label = "mousemove"];
    "not\ndragging" -> dragging [label = "mousedown"];
    dragging -> dragging [label = "mousemove"];
    dragging -> "not\ndragging" [label = "mouseup"];
}
#+end_src

#+results[3ae559af3a5b93dfa534a4a0bc7bb009f8a5ddcd]:
...
*Try* the demo with a mouse: @@html:<strong class="on-touch-device">(won't work well on touch devices)</strong>@@
...
digraph {
    ...
    dragging -> "not\ndragging" [label = "document\nmouseup"];
}
#+end_src

#+results[0b2028ca9374d050d7561e76610f628578a71da0]:
[[file:build/mouse-document.svg]]

#+begin_export html
<pre data-code="mouseGlobal" />
#+end_export
Try it out. It works better. @@html:<strong class="on-touch-device">(only with mouse, not touch)</strong>@@
...
digraph {
    ...
    rankdir=LR;
    "not\ndragging" -> "not\ndragging" [label = "touchmove"];
    "not\ndragging" -> dragging [label = "touchstart"];
    dragging -> dragging [label = "touchmove"];
    dragging -> "not\ndragging" [label = "touchend"];
    dragging -> "not\ndragging" [label = "touchcancel"];
}
#+end_src

#+results[28e17f8b1a41b54a0019a6e1db9c4ade0f028068]:
...
*Try* the demo with a touch device: @@html:<strong class="on-mouse-device">(won't work with a mouse)</strong>@@
...
#+results[5de7e4f1b5907af5dd9ae40815f99316b7a04ead]:
...
digraph {
    ...
    rankdir=LR;
    "not\ndragging" -> "not\ndragging" [label = "pointermove"];
    "not\ndragging" -> dragging [label = "pointerdown"];
    dragging -> dragging [label = "pointermove"];
    dragging -> "not\ndragging" [label = "pointerup"];
    dragging -> "not\ndragging" [label = "pointercancel"];
}
#+end_src

#+results[09b2cb93f1459a9e2971e35ca1643e60a26432a1]:
...
*Try* dragging the circle on a touch device.  @@html:<em class="on-mouse-device">(The issue won't show with mouse dragging.)</em>@@ It shouldn't scroll. But then try scrolling by dragging the diagram. It doesn't scroll either, but I want it to. I want to stop scrolling /only/ if dragging the circle, not when dragging the diagram.
...
*Try* dragging quickly with the mouse. 
*Try* selecting all text on the page to see whether the text inside the circle is selectable when not dragging.
...
I think either Circle 2 or Circle 3's behavior is a reasonable choice. Note that as of early 2023, Safari /still/  [[https://caniuse.com/?search=user-select][doesn't support the unprefixed version]] ([[https://bugs.webkit.org/show_bug.cgi?id=208677][tracking bug]]), so we have to also set the prefixed version.

#+begin_comment
Programatically setting the selection in Firefox selects the text that is supposed to be unselectable with user-select:none! So I didn't implement that button.
#+end_comment
...
digraph {
    ...
    "neither\nbutton" -> leftbutton [label = "pointerdown\nleft"];
    "neither\nbutton" -> rightbutton [label = "pointerdown\nright"];
    leftbutton -> "neither\nbutton" [label = "pointerup\nleft"];
...
#+results[322071b4e259e358d6e86dff35b5c98945102983]:
...
I have a [[href:examples.html][demo]] on the examples page.
...
For a demo, see my [[https://www.redblobgames.com/making-of/responsive-design/#visualizations][Responsive Design page]]. With a mouse, you can hover over the rows to change the layout. With touch, you can drag to change the layout. I also do this on the [[href:/grids/hexagons/][Hexagons Guide]]. Many diagrams work with mouse hover, but on touch devices they work with touch drag.
...
** Variant: toggle paint

For the [[https://www.redblobgames.com/x/1825-rounded-cell-painter/][Rounded Cell Painter]], the =pointerdown= captures the initial paint color, and then all =pointermove= events will use that same paint color, until =pointerup=. To do this, the =state.dragging= variable should contain not only the initial =x=, =y= but also the initial paint color.

I have a demo on the examples page.

** Edge case: lostpointercapture

I know that =lostpointercapture= can be used to detect that we lost pointer capture, but I haven't yet figured out all the situations in which it fires, and what I should do about them. In the comments, [[href:#comment-6606271359][Kirill Kleimenov gives this scenario]]:

1. Start dragging a circle with left button
2. Get outside the drag area holding left button down
3. Press right button, then release left button
4. Click anywhere outside drag area

The [[https://www.w3.org/TR/pointerevents/#the-lostpointercapture-event][standard]] says "The user agent MUST fire a pointer event named =lostpointercapture= after pointer capture is released for a pointer" but browser behavior varies quite a bit. In the "I made some notes" expandable section in the [[href:#fix-contextmenu][Fix: context menu]] section of this page, I list a /lot/ of platform-specific and browser-specific behavior when it comes to the right mouse button. In some of these, =lostpointercapture= fires, but in some systems it will not fire until you click elsewhere.
...
<style>
  ...
  details { padding: 0 1em; background: linear-gradient(to right, hsl(200 10% 95%), white); }
  ...
  .draggable { cursor: grab; font-family: var(--sans-serif); }
  .draggable.dragging { cursor: grabbing; }
  .draggable circle { fill: hsl(0 50% 50%); stroke: hsl(0 100% 30%); }
  .draggable.bad circle { fill: hsl(30 50% 50%); stroke: hsl(30 100% 30%); }
  .draggable.good circle { fill: hsl(150 50% 45%); stroke: hsl(150 100% 30%); }
  .draggable.dragging circle { fill: hsl(200 50% 50%); stroke: hsl(200 100% 30%); }
  ...
  #scrubbable-number:hover {
     background: hsl(30 50% 90%);
  }
  #scrubbable-number:focus-visible {
     background: hsl(0 50% 90%);
     outline: none;
  }

  #final-code-options label { display: block; }
  ...
  /* attempt to detect touch vs mouse, for highlighting which examples will work */
  @media (hover: hover) { .on-touch-device { display: none; } }
  @media (hover: none)  { .on-mouse-device { display: none; } }
</style>
...
<x:footer>
  ...
  <!-- hhmts start -->Last modified: 12 Dec 2024<!-- hhmts end -->
</x:footer>
#+end_export
