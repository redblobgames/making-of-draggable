...
3. snap to grid, like jquery-ui and react-draggable
4. invisible larger drag area for mobile (from my little-details page)
5. the drag + resize example from the main page, which needs =stopPropation()=
6. constrain position to a non-rectangular area, like my [[href:/articles/curved-paths/making-of.html][making-of curved-paths page]]
7. accessibility? - make the numbers also show up in an input box, or make things draggable by keyboard?
8. some way to cancel a drag with keyboard
9. .dragging class options (from little-details page)
