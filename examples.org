...
5. snap to grid, like jquery-ui and react-draggable
6. invisible larger drag area for mobile (from my little-details page)
7. the drag + resize example from the main page, which needs =stopPropation()=
8. constrain position to a non-rectangular area, like my [[href:/articles/curved-paths/making-of.html][making-of curved-paths page]]
9. accessibility? - make the numbers also show up in an input box, or make things draggable by keyboard?
10. some way to cancel a drag with keyboard
11. .dragging class options (from little-details page)
