...
2. snap to grid, like jquery-ui and react-draggable, although this isn't any different from the svg-snapping example in theory
3. invisible larger drag area for mobile (from my little-details page)
4. the drag + resize example from the main page, which needs =stopPropation()=
5. constrain position to a non-rectangular area, like my [[href:/articles/curved-paths/making-of.html][making-of curved-paths page]]
6. some way to cancel a drag with keyboard → probably outside the scope
7. .dragging class options (from little-details page)
8. multiple draggables that are linked somehow, e.g. maybe they share a y but have independent x
9. accessibility? - make the numbers also show up in an input box, or make things draggable by keyboard?
...
<x:footer>
  ...
  <!-- hhmts start -->Last modified: 19 Feb 2024<!-- hhmts end -->
</x:footer>
#+end_export
