...
*These are starting points* using global variables. You'll have to adapt the code to fit into your own project.

{ TODO: describe how the event handler and state handler relate }

#+begin_export html
<show-example name="svg-circle" />
#+end_export

{ TODO: consider splitting the script code into the state handler and the event handler code }
...
<style>
  ...
  show-example > pre { width: unset; font-size: clamp(0.5rem, 1.2vw, 1rem); border: none; }
...
<x:footer>
  ...
  <!-- hhmts start -->Last modified: 20 Dec 2023<!-- hhmts end -->
</x:footer>
#+end_export
