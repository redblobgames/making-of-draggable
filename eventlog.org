...
[[https://bugs.chromium.org/p/chromium/issues/detail?id=1264930]] → a =contextmenu= should produce a PointerEvent that has =.pointerId= the same as the id of the event that produced that menu. It is fixed on Chrome but it is undefined on Firefox, because Firefox 110 produces a MouseEvent not a PointerEvent.
